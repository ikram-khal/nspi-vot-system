...
export default function MeetingDetailPage() {
  ...
  // Poll for live updates on voting questions
  useEffect(() => {
  ...
  const addQuestion = async () => {
    ...
    setNewQuestion('');
    toast.success('Мәселе қосылды');
    load();
  };

  const deleteQuestion = async (qId: string) => {
    if (!confirm('Мәселени өширесиз бе?')) return;
    await supabase.from('questions').delete().eq('id', qId);
    load();
  };

  const startVoting = async (qId: string) => {
    if (attendeeIds.size === 0) { toast.error('Алдын ала қатнасыўшыларды белгилең'); return; }
    await supabase.from('questions').update({ status: 'voting' }).eq('id', qId);
    toast.success('Даўыс бериў басланды!');
    load();
  };

  const stopVoting = async (qId: string) => {
    await supabase.from('questions').update({ status: 'closed' }).eq('id', qId);
    toast.success('Даўыс бериў тоқтатылды');
    load();
  };
  ...
  const handleReport = async () => {
    ...
    toast.success('Есабат жүклеп алынды');
  };

  if (loading) return <div className="text-center py-12 text-muted-foreground">Жүкленбекте...</div>;
  if (!meeting) return <div className="text-center py-12">Мәжилис табылмады</div>;

  const statusIcon = (s: string) => s === 'draft' ? '📝' : s === 'voting' ? '🟢' : '🔴';
  const verdict = (q: Question) => {
    if (q.votes_for > q.votes_against) return 'ҚАРАР ҚАБЫЛ ЕТИЛДИ';
    if (q.votes_for < q.votes_against) return 'ҚАРАР ҚАБЫЛ ЕТИЛМЕДИ';
    return 'ДАЎЫСЛАР ТЕҢ';
  };
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold">Мәжилис №{meeting.protocol_number}</h2>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-base">Қатнасыўшылар ({attendeeIds.size} / {members.length})</CardTitle>
          <Button variant="outline" size="sm" onClick={selectAll}>Барлығын белгилеў</Button>
        </CardHeader>
        <CardContent>
          ...
          {totalPages > 1 && (
            <div className="flex gap-2 mt-3 justify-center">
              {Array.from({ length: totalPages }, (_, i) => (
                <Button key={i} variant={attendeePage === i ? 'default' : 'outline'} size="sm" onClick={() => setAttendeePage(i)}>
                  {i + 1}
                </Button>
              ))}
            </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Даўысқа қойылған мәселелер</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex gap-2">
            <Input placeholder="Жаңа мәселе текстин киргизиң" value={newQuestion} onChange={e => setNewQuestion(e.target.value)} className="flex-1" />
            <Button onClick={addQuestion}>Қосыў</Button>
          </div>

          {questions.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">Мәселелер жоқ</p>
          ) : (
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center justify-between">
                          <span className="text-sm text-muted-foreground">
                            {q.voted_count} / {attendeeIds.size} даўыс берди
                          </span>
                          <Button size="sm" variant="destructive" onClick={() => stopVoting(q.id)}>
                            ⏹️ Тоқтатыў ({attendeeIds.size - q.voted_count} даўыс бермеди)
                          </Button>
                        </div>
                      </div>
                    )}
